...
- **대가:** ① offset이 저장 내부(byte 위치)를 컨슈머에 노출 ② 컨슈머가 레코드 경계 아닌 offset을 보내면 길이 헤더를 payload 중간에서 읽어 쓰레기가 나옴 → "produce/이전 fetch가 준 offset만 쓴다"는 암묵적 계약 ③ 카프카의 논리 ID 의미론 포기 ④ **리텐션과 양립 불가** — byte-offset은 물리 위치라 오래된 데이터를 삭제/압축하면 뒤 레코드의 offset이 밀린다. v1은 삭제를 안 해서 안전하지만, v2 리텐션을 하려면 offset을 논리번호+인덱스로 바꿔야 한다(byte-offset과 삭제는 공존 불가).
- **참고:** 순번 offset은 논리 ID라 삭제해도 밀리지 않는다(카프카). 우리 byte-offset은 물리 위치라 삭제하면 밀리지만, v1은 삭제 자체를 안 하므로 이 방식이 성립한다.
